...
export const PkmList = Loadable({
  loader: () =>
    import(/* webpackChunkName: "formatters/handlebars" */ './components').then(e => e.PkmList),
  loading: Loading,
});
export const FormattingOptions = Loadable({
  loader: () =>
    import(/* webpackChunkName: "formatters/handlebars" */ './components').then(
...
export const defaultOptions = [
  {
    ...
    format: {
      ...
      format:
        '{{#if isGhost}}~{{/if}}B{{box}} - {{row}},{{column}} - {{speciesName}} ({{genderString gender}}) - {{natureName}} - {{abilityName}} - {{ivHp}}.{{ivAtk}}.{{ivDef}}.{{ivSpAtk}}.{{ivSpDef}}.{{ivSpe}} - {{typeName hpType}} [{{esv}}]',
      boxHeader:
        '<h3 style="margin-bottom: 0; font-family: Roboto, sans-serif; color: #999">Box {{box}}</h3>',
      splitBoxes: false,
    },
  },
  {
    ...
    format: {
      ...
      format:
        '| {{#if isGhost}}~{{/if}}B{{box}} | {{row}},{{column}} | {{ot}} | {{tid6}} | {{sid}} | {{tsv}} |',
      boxHeader:
        '<h3 style="margin-bottom: 0; font-family: Roboto, sans-serif; color: #999">Box {{box}}</h3>',
      splitBoxes: false,
    },
  },
  {
    ...
    format: {
      ...
      format:
        '| {{#if isGhost}}~{{/if}}B{{box}} | {{row}},{{column}} | {{ot}} | {{tid}} | {{tsv}} |',
      boxHeader:
        '<h3 style="margin-bottom: 0; font-family: Roboto, sans-serif; color: #999">Box {{box}}</h3>',
      splitBoxes: false,
    },
  ...
  },
  {
    ...
    format: {
      header:
        'Box,Row,Column,Species,Gender,Nature,Ability,HP IV,ATK IV,DEF IV,SPA IV,SPD IV,SPE IV,HP Type,ESV,TSV,Nickname,OT,Ball,TID,SID,HP EV,ATK EV,DEF EV,SPA EV,SPD EV,SPE EV,Move 1,Move 2,Move 3,Move 4,Relearn 1,Relearn 2,Relearn 3,Relearn 4,Shiny,Egg',
      format:
        '{{#if isGhost}}~{{/if}}{{box}},{{row}},{{column}},{{species}},{{gender}},{{nature}},{{ability}},{{ivHp}},{{ivAtk}},{{ivSpAtk}},{{ivSpDef}},{{ivSpe}},{{hpType}},{{esv}},{{tsv}},{{nickname}},{{ot}},{{ball}},{{tid6}},{{sid}},{{evHp}},{{evAtk}},{{evDef}},{{evSpAtk}},{{evSpDef}},{{evSpe}},{{move1}},{{move2}},{{move3}},{{move4}},{{eggMove1}},{{eggMove2}},{{eggMove3}},{{eggMove4}},{{isShiny}},{{isEgg}}',
      boxHeader: '<div style="margin-bottom: 20px;"></div>',
      splitBoxes: false,
    },
  },
