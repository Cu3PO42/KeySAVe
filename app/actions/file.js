...
export const openFile = createAction(OPEN_FILE, async (files) => {
  ...
  const { reader } = res;
  if (res.type === 'SAV') {
    return { pokemon: reader.getAllPkx(), keyProperties: reader.isNewKey, type: 'SAV', name: files, generation: reader.generation };
  } else {
    return { pokemon: reader.getAllPkx(), keyProperties: reader.workingKeys, type: 'BV', name: files, generation: reader.generation };
  }
});
